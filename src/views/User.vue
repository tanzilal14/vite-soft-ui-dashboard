<template>
    ...
    <div class="row">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-9">
                        <Datepicker v-model="date" range auto-apply :enable-time-picker="false" :format="formatpicker"></Datepicker>
                    </div>
                </div><br>
                <div class="row">
                    <label class="col-form-label col-2">Email :</label>
                    <div class="col-9">
                        <input v-model="filter.email" type="text" class="form-control" />
                    </div>
                </div><br>
                <div class="row">
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    <div>
                    <button class="btn mb-0 bg-gradient-danger btn-sm null null ms-auto mb-0 pull-right" @click="resetFilter">Reset</button>&nbsp;
                    <button class="btn mb-0 bg-gradient-success btn-sm null null ms-auto mb-0 pull-right" @click="fetchData">Search</button>
                    </div>
                </div>
            </div>
        </div>
      ...
      <div class="col-12">
        <div class="card mb-4">
            <div class="card-header pb-0 mb-3">
                <div class="d-flex align-items-center">
                    <h6 class="mb-0 text-success">List User</h6>
                ...
                <div class="table-responsive">
                    <vue-good-table
                        ...
                        <template #table-row="props" :index="index" :columns="columns">
                            ...
                            <span v-else-if="props.column.field === 'isActive'">{{ props.formattedRow[props.column.field] ? 'Yes' : 'No' }}</span>
  ...
  <div id="modal-form" ref="formModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <Form ref="form" v-slot="{ errors }" role="form" method="POST" :validation-schema="schema" class="needs-validation" @submit="save">
          <div class="modal-header">
            <h5 id="formModalLabel" class="modal-title text-success">Form User</h5>
          ...
          <div class="modal-body">
              <div class="form-group">
                ...
                <Field id="name" v-model="form.name" type="text" name="name" :class="'form-control '+(errors.name ? 'is-invalid' : '')" />
              ...
              <div class="form-group">
                ...
                <Field id="price" v-model="form.price" name="price" :class="'form-control thousand '+(errors.price ? 'is-invalid' : '')" />
          ...
          <div class="modal-footer">
            ...
            <button v-if="!loading" type="submit" :disabled="loading" class="btn bg-gradient-success">Save</button>
            <button v-else class="btn bg-gradient-success" type="button" :disabled="loading">
  ...
  methods : {
        ...
        if(self.file !== null){
            await self.$apollo.mutate({
                ...
                if(result.data.uploadImages[0].filename !== undefined){
                    self.form.image = result.data.uploadImages[0].filename;
                }
            }).catch((error) => {
    ...
    fetchData() {
        ...
        this.$apollo.query({
            ...
            self.totalRecords = result.data.users.totalCount;
            self.isLoading = false;
            self.rows = result.data.users.nodes;
        }).catch((error) => {
